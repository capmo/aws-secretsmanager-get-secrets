 ...
 */
export async function buildSecretsList(
  ...
  for (const configInput of configInputs) {
    if (configInput.includes("*")) {
      const [secretAlias, secretPrefix] =
        extractAliasAndSecretIdFromInput(configInput);

      if (!validFilter.test(secretPrefix)) {
        throw new Error(
          "Please use a valid prefix search (should be at least 3 characters and end in *)"
        );
      }

      // Find and add results for a given prefix
      const prefixMatches: string[] = await getSecretsWithPrefix(
        client,
        secretPrefix,
        !!secretAlias
      );
      ...
      prefixMatches.forEach((secret) =>
        finalSecretsList.add(secretAlias ? `${secretAlias},${secret}` : secret)
      );
    } else {
 ...
 */
export async function getSecretsWithPrefix(
  client: SecretsManagerClient,
  prefix: string,
  hasAlias: boolean
): Promise<string[]> {
  ...
  const response: ListSecretsResponse = await client.send(
  ...
  if (response.SecretList) {
    ...
    if (secretsList.length === 0) {
      throw new Error(
        `No matching secrets were returned for prefix "${prefix}".`
      );
    } else if (hasAlias && secretsList.length > 1) {
      // If an alias was requested, we cannot match more than one result
      throw new Error(
        `A unique alias was requested for prefix "${prefix}", but the search result for this prefix returned multiple results.`
      );
    } else if (response.NextToken) {
      // If there is a second page of results, this exceeds the max number of matches
      throw new Error(
        `A search for prefix "${prefix}" matched more than the maximum of ${LIST_SECRETS_MAX_RESULTS} secrets per prefix.`
      );
    }
...
export function injectSecret(
  ...
  if (parseJsonSecrets && isJSONString(secretValue)) {
    ...
    const secretMap = JSON.parse(secretValue) as Record<
      string,
      string | object
    >;
    ...
    for (const k in secretMap) {
      const keyValue =
        typeof secretMap[k] === "string"
          ? (secretMap[k] as string)
    ...
    if (process.env[envName] || envName === CLEANUP_NAME) {
      console.warn(
        `The environment name '${envName}' is already in use. It WILL BE overwritten. Please use an alias to ensure that each secret has a unique environment name`
      );
    }
...
export function isSecretArn(secretId: string): boolean {
  const validArn = new RegExp(
    "^arn:aws:secretsmanager:.*:[0-9]{12,}:secret:.*$"
  );
 ...
 */
export function extractAliasAndSecretIdFromInput(
  input: string
